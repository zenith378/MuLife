...
void energy_df()
{
   ...
   tree1->ReadFile(run1, "time/D:E_in_v/D:E_fin_v");
   tree1->ReadFile(run0);
   ...
   // endpoint = 52.8 MeV
   TH1F *h1 = new TH1F("h1", "h1", 50, 0.1*77.44, 1.5*77.44);
   ...
   auto df_cut = df_new.Filter("E_fin>0.", "Final energy above 0.")
                     .Filter("E_in<0.7*77.44", "Initial energy below 0.7")
   ...
   auto df_cor = df_cut.Define("E_fin_cor", "E_fin+0.0838*E_in")
                     ...
                     .Filter("E_cor>0.05*77.44", "Overall corrected energy above 0");
   ...
   // gPad->SetLogy();
   auto h = df_cor.Histo1D({"h", "Electron Energy Spectrum", 50, 0.1*77.44, 1.5*77.44}, "E_cor");

   h->SetBinErrorOption(TH1::EBinErrorOpt::kPoisson);
   h1->GetYaxis()->SetTitle("Counts");
   h1->GetXaxis()->SetTitle("Voltage [V]");
   h1->SetTitle("Energy in target E_{fin}>0. && 0.<E_{in}<0.7 and E_{tot}>0.1");
   h->SetMarkerStyle(8);
